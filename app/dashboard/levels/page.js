...
export default function Page() {
  ...
  return (
    <div className="grid grid-cols-3 gap-3">
      {courses.map((itm, idx) => (
        <Link href={`/dashboard/levels/${itm.title}?id=${itm.id}`}>
          <div
            key={itm.id}
            className="border-2 border-muted rounded-lg p-2 cursor-pointer hover:border-muted-foreground transition-colors"
          >
